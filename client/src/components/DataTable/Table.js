...
const Hoge = ({ columns, data, WBid, tableType }) => {
...
const Table = ({ WBid, tableType }) => {
  ...
  useEffect(() => {
    ...
    fetch(url)
      ...
      .then((json) => {
        console.log(json);
        return json[`${tableType}`];
      })
      .then((jsonSpecific) => {
        console.log(jsonSpecific);
        setData(jsonSpecific.data);
      });
  }, [WBid, tableType]);
  ...
  return (
    <Hoge columns={columns} data={data} WBid={WBid} tableType={tableType} />
  );
};
